...
document.title = props.existingResep ? "Edit Resep" : "Buat Resep Baru";

const resep = ref({
  id_resep: props.existingResep['resep']?.id_resep || null,
  judul: props.existingResep['resep']?.judul || '',
  gambar: null,
  ...
  bahan: props.existingResep['bahan']?.map(b => ({
    nama_bahan: b.nama_bahan,
    jumlah_bahan: b.jml_bahan,
    satuan_bahan: b.stn_bahan
  })) || [{ nama_bahan: '', jumlah_bahan: '', satuan_bahan: 'Buah' }],
  langkah: props.existingResep['langkah']?.map(l => ({
    urutan: l.urutan,
    cara_langkah: l.cara_langkah
  })) || [{ cara_langkah: '' }],
});
...
const tambahBahan = () => {
  resep.value.bahan.push({ nama_bahan: '', jumlah_bahan: '', satuan_bahan: 'Buah' });
};
...
const tambahLangkah = () => {
  resep.value.langkah.push({
    urutan: resep.value.langkah.length + 1,
    cara_langkah: ''
  });
};
...
const hapusLangkah = (i) => {
  ...
  // Update urutan if needed
  resep.value.langkah.forEach((l, idx) => (l.urutan = idx + 1));
};

const submitResep = async () => {
  const formData = new FormData();
  formData.append('_method', 'PUT');
  formData.append('id_resep', resep.value.id_resep);
  formData.append('judul', resep.value.judul);
  formData.append('deskripsi', resep.value.deskripsi);
  if (resep.value.gambar) {
  ...
  try {
    const url = resep.value.id_resep ? '/api/updateresep' : '/api/addresep';
  ...
  <App>
    <div class="container my-3">
      <form @submit.prevent="submitResep" enctype='multipart/form-data'>
        <h2>{{ resep.id_resep ? 'Edit Resep' : 'Buat Resep Baru' }}</h2>
        ...
        <div class="row">
          <div class="col-12 col-md-5">
            ...
            <div class="input-group mb-3">
              ...
              <div v-if="existingResep?.gambar" class="mt-2">
                <img :src="`/storage/${existingResep.gambar}`" width="100" class="img-thumbnail" />
          ...
          <div class="col-12 col-md-7">
            <div class="row">
              <div class="col-6">
                ...
                <div class="input-group mb-3">
                  <input v-model="resep.waktu_masak" type="number" min="1" max="360" class="form-control"
                    placeholder="10 Menit" required />
                </div>
              </div>
              <div class="col-6">
                <label class="form-label h6">Porsi Masak</label>
                <div class="input-group mb-3">
                  <input v-model="resep.porsi_masak" type="number" min="1" max="35" class="form-control"
                    placeholder="5 Orang" required />
                </div>
              </div>
            </div>
            <!-- Alat -->
            ...
            <button type="submit" class="btn btn-success">
              {{ resep.id_resep ? 'Update Resep' : 'Simpan Resep' }}
            </button>
          </div>
        </div>
      </form>
